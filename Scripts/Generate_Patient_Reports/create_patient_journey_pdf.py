...
def get_child_nodes(session, parent_node):
    ...
    as they are auxiliary nodes for embeddings/search purposes"""
...
def sanitize_html_tags(text):
    ...
    Specifically fixes <br> tags to be self-closing <br/>.
    """
    if not text:
        return text
    
    # Replace <br> with <br/> to make them self-closing
    text = text.replace('<br>', '<br/>')
    text = text.replace('<BR>', '<br/>')
    
    # Also handle cases where there's already a space before the closing
    text = text.replace('<br />', '<br/>')
    text = text.replace('<BR />', '<br/>')
