...
# Suppress Neo4j driver logs and notifications
...
# ═══════════════════════════════════════════════════════════════
# CONFIGURATION: Multiple API Keys with Automatic Rotation
# ═══════════════════════════════════════════════════════════════
API_KEYS = [
    'AIzaSyDG9VKp-kD1y0-xT-5ivV7Ldni-YDR-wOk',
    'AIzaSyBhtoYXB1Ta50iO5Is5Um0IM10mAfuZElg',  # Primary key
    'AIzaSyDE5Ck37g-cNIL9WCu_PIT91q1_ySuhNRY',  # Backup key 1
    'AIzaSyDGk2xP53Zv3F7B-ciKf-1a-YtW6RF-l04',  # Backup key 2
    'AIzaSyDkxtiidxAyAh_w0lfnZRB9w_BE3-Ew6gE'   # Backup key 3
]
# ═══════════════════════════════════════════════════════════════
...
def switch_api_key():
    ...
    logger.info(f"[API KEY SWITCH] Switching from key #{old_index + 1} to key #{current_key_index + 1}")
    return True
...
def is_invalid_key_error(error):
    """Check if the error is an invalid API key error"""
    error_str = str(error).lower()
    return 'api key not valid' in error_str or 'api_key_invalid' in error_str
...
def extract_graph_structure(session, subject_id):
    ...
    # Exclude item nodes (DiagnosisItem, MedicationItem, LabResultItem, MicrobiologyResultItem)
    # as they are auxiliary nodes for embeddings/search and redundant for summarization
    query = """
    MATCH (p:Patient)
    ...
    # Warn if patient has no relationships
    if len(graph_data['relationships']) == 0:
        logger.warning(f"Patient {subject_id} has no relationships in the graph - only patient demographics available")
...
def get_llm_summary(graph_data, subject_id, max_retries=3):
    """Send graph structure to LLM and get structured JSON summary with automatic key rotation"""
...
Return ONLY the JSON object without any additional text or formatting:"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"[API KEY] Using key #{current_key_index + 1} (Attempt {attempt + 1}/{max_retries})")
            ...
            if is_invalid_key_error(e):
                ...
                # Try switching to next key immediately
            ...
            if is_rate_limit_error(e):
                ...
                if len(keys_tried) < len(API_KEYS):
                    if switch_api_key():
                        ...
                        raise Exception("All API keys hit rate limits")
                else:
                    raise Exception("All API keys hit rate limits")
    ...
    raise Exception(f"Failed to get LLM summary after {max_retries} attempts")
