...
def create_microbiology_nodes(tracker: Optional[ETLTracker] = None, pipeline_log_file: Optional[str] = None):
    ...
    if pipeline_log_file:
        ...
        log_file = logs_dir / 'temp_add_micro_biology_events.log'
    ...
    project_root = script_dir.parent.parent.parent
    MICROEVENTS_CSV = project_root / 'Filtered_Data' / 'hosp' / 'microbiologyevents.csv'
    ...
    # Load CSV
    microevents_df = pd.read_csv(str(MICROEVENTS_CSV))
    ...
    try:
        with neo4j_conn.session() as session:
            ...
            pbar = tqdm(total=len(event_list), desc="Adding microbiology event nodes", unit="event")
            ...
            for record in event_list:
                ...
                pbar.update(1)
                pbar.set_postfix({'Processed': processed_count, 'Skipped': skipped_count, 'Failed': len(failed_patients), 'Tracked': len(patients_tracked_this_run)})
