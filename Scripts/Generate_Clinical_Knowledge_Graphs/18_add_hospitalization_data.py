...
def create_admission_vitals_node(session, row):
    ...
    params = {'hadm_id': hadm_id, 'vitals_id': vitals_id}
...
def create_admission_labs_node(session, row):
    ...
    result = session.run(query, hadm_id=hadm_id, labs_id=labs_id, lab_tests=labs_list)
    record = result.single()
    return record and record['admission_exists']
...
def create_admission_medications_node(session, row):
    ...
    result = session.run(query, hadm_id=hadm_id, med_id=med_id, medications=medications_list)
    record = result.single()
    return record and record['admission_exists']
