...
# Add Create_Embeddings directory to path for local imports (for lazy imports in methods)
create_embeddings_dir = Path(__file__).parent
if str(create_embeddings_dir) not in sys.path:
            ...
            if self.use_openai:
                ...
                if use_chunking:
                    ...
                    if len(chunks) == 1:
                        ...
                        return self.model.encode(chunks[0], convert_to_numpy=True, show_progress_bar=False)
                    else:
